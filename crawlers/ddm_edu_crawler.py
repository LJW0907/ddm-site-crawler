...
class DDMEducationCrawler:
    ...
    def _crawl_sorted_board(self, params, parser_func, content_type):
        ...
        if self.test_mode:
            print(f"   [TEST MODE] {self.date_threshold} 이후 데이터 수집")

        items = []
        page = 1
        max_pages = 10 if self.test_mode else 5  # 테스트 모드에서는 더 많은 페이지 확인
            ...
            try:
                ...
                for row in rows:
                    ...
                    if not is_valid_date:
                        if not self.test_mode:  # 일반 모드에서만 중단
                            stop_for_this_board = True
                            break
                    else:
                        items.append(item)

                if stop_for_this_board and not self.test_mode:
                    break
    ...
    def _crawl_unsorted_board(self, params, parser_func, content_type):
        ...
        all_items = []
        page = 1
        max_pages = 10 if self.test_mode else 5  # 테스트 모드에서는 더 많은 페이지
    ...
    def _crawl_notices(self, params, parser_func, content_type):
        ...
        # 테스트 모드: 3개월, 일반 모드: 2개월
        if self.test_mode:
            months_ago = self.today - relativedelta(months=3)
            print(f"   [TEST MODE] 3개월 전까지 수집")
        else:
    ...
    def crawl_all(self):
        ...
        results["test_mode"] = self.test_mode  # 테스트 모드 표시
        return results
...
if __name__ == "__main__":
    ...
    # python crawlers/ddm_edu_crawler.py
